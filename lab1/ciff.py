    ...
    def parse_ciff_file(file_path):
        ...
        try:
            with open(file_path, "rb") as ciff_file:
                ...
                # check if magic contains 4 bytes
                if len(magic) != 4:
                    raise Exception("Invalid magic number length")
                bytes_read += 4
                
                # decode the bytes as 4 characters
                new_ciff.magic = magic.decode('ascii')
                # check if the magic is "CIFF"
                ...
                # check if header size contains 8 bytes
                if len(h_size) != 8:
                    raise Exception("Invalid header size length")
                bytes_read += 8
                ...
                # check if content size contains 8 bytes
                if len(c_size) != 8:
                    raise Exception("Invalid content size length")
                bytes_read += 8
                # interpret the bytes as an 8-byte-long integer
                # HINT: check out the "q" format specifier!
                # HINT: Does it fit our purposes?
                new_ciff.content_size = struct.unpack("q", c_size)[0]
                # check if the content size is in the valid range
                ...
                if len(width) != 8:
                    raise Exception("Invalid width length")
                bytes_read += 8
                ...
                # check if the width is in the valid range
                ...
                if len(height) != 8:
                    raise Exception("Invalid height length")
                bytes_read += 8
                ...
                # check if the height is in the valid range
                if new_ciff.height < 0 or new_ciff.height > 2**64 - 1:
                    raise Exception("Height out of valid range")

                # check if the content size matches the width*height*3
                ...
                while char != '\n':
                    ...
                    # check if c contains 1 byte
                    if len(c) != 1:
                        raise Exception("Invalid caption character length")
                    bytes_read += 1
                    char = c.decode('ascii')
                ...
                while bytes_read != new_ciff.header_size:
                    ...
                    if char == '\n':
                        raise Exception("Tag contains newline character")
                    ...
                    if (bytes_read == new_ciff.header_size) and (tag != "" and tag[-1] != '\0'):
                        raise Exception("The last character of the header must be a '\\0'")
                
                # all tags must end with '\0'
                # check the end of each tag for the '\0'
                for tag in tags:
                    if tag[-1] != '\0':
                        raise Exception("Each tag must end with a '\\0'")
                ...
                while bytes_read < new_ciff.header_size+new_ciff.content_size:
                    ...
                    if len(c) != 3:
                        raise Exception("Invalid pixel length")
                    bytes_read += 3
                ...
                # check if there is an extra byte at the end of the file
                if extra_byte:
                    raise Exception("Extra data found at the end of the file")
